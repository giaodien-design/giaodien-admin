...
export default function AddScreensPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [appResult, flowsResult] = await Promise.all([getAppById(appId), getAllFlows()]);
    ...
    <div className="max-w-4xl mx-auto w-full py-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Upload screenshots and assign them to flows (optional)</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Upload screenshots and select which flow they belong to</p>
              </div>
              <MultiImageUpload onScreensChange={setScreens} maxFiles={20} flows={flows} />
            </div>
